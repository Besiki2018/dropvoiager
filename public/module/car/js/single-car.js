(function ($) {
    new Vue({
        ...
        computed:{
            ...
            topPriceDisplay:function () {
                var summary = this.priceSummary;
                if (summary && summary.total) {
                    return summary.total;
                }
                if (this.base_display_price) {
                    return this.base_display_price;
                }
                var meta = this.pricing_meta || {};
                var mode = (meta.mode || '').toLowerCase();
                if (mode === 'fixed') {
                    var perUnit = this.toNumeric(meta.fixed_price);
                    var baseFee = this.toNumeric(meta.base_fee);
                    if (perUnit !== null) {
                        var total = perUnit;
                        if (baseFee !== null) {
                            total += baseFee;
                ...
                var baseValue = this.toNumeric(this.base_price_value);
                if (baseValue !== null) {
                    return this.formatMoney(baseValue);
                }
                return '';
            },
            topSaleDisplay:function () {
                if (this.priceSummary && this.priceSummary.total) {
                    return '';
                }
                return this.base_display_sale_price || '';
            },
            topPriceHasSale:function () {
                var sale = this.topSaleDisplay;
                return !!(sale && sale.length);
            },
