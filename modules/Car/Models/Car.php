...
class Car extends Bookable
{
    ...
    protected $casts = [
        ...
        'price'=>'float',
        'sale_price'=>'float',
        'transfer_service_lat' => 'float',
        'transfer_service_lng' => 'float',
        'transfer_service_radius_km' => 'float',
        'transfer_base_radius_km' => 'float',
        'transfer_base_price' => 'float',
        'transfer_price_per_km' => 'float',
    ];
    /**
     * @var Booking
     */
    ...
    protected $tmp_dates = [];
    public ?array $transfer_quote = null;
    ...
    public function setTransferQuote(?array $quote): void
    {
        $this->transfer_quote = $quote ?: null;
    }

    public function getTransferQuoteAttribute(): ?array
    {
        return $this->transfer_quote;
    }

    protected function normalizeCoordinate(?array $point): ?array
    {
        if (!is_array($point)) {
            return null;
        }
    ...
    protected function distanceInKm(float $latFrom, float $lngFrom, float $latTo, float $lngTo): float
    {
        ...
        $a = sin($deltaLat / 2) ** 2 + cos($latFromRad) * cos($latToRad) * sin($deltaLng / 2) ** 2;
        $c = 2 * atan2(sqrt($a), sqrt(1 - $a));

        return $earthRadius * $c;
    }

    public function distanceFromServiceCenter(?float $lat, ?float $lng): ?float
    {
        if ($lat === null || $lng === null || $this->transfer_service_lat === null || $this->transfer_service_lng === null) {
            return null;
        }

        return $this->distanceInKm($this->transfer_service_lat, $this->transfer_service_lng, $lat, $lng);
    }

    public function calculateTransferQuote(?array $pickup, ?array $dropoff, ?float $routeDistanceKm): ?array
    {
        $pickupPoint = $this->normalizeCoordinate($pickup);
        if (!$pickupPoint || $this->transfer_base_price === null) {
            return null;
        }

        $pickupDistance = $this->distanceFromServiceCenter($pickupPoint['lat'], $pickupPoint['lng']);
        if ($pickupDistance === null) {
            return null;
        }

        if (!empty($this->transfer_service_radius_km) && $pickupDistance > $this->transfer_service_radius_km) {
            return null;
        }

        $dropoffPoint = $this->normalizeCoordinate($dropoff);
        $dropoffDistance = null;
        if ($dropoffPoint) {
            $dropoffDistance = $this->distanceFromServiceCenter($dropoffPoint['lat'], $dropoffPoint['lng']);
        }

        $effectiveDistance = $routeDistanceKm;
        if ($effectiveDistance === null && $dropoffPoint) {
            $effectiveDistance = $this->distanceInKm(
                $pickupPoint['lat'],
                $pickupPoint['lng'],
                $dropoffPoint['lat'],
                $dropoffPoint['lng']
        ...
        $baseRadius = $this->transfer_base_radius_km ?? 0.0;
        $basePrice = $this->transfer_base_price ?? 0.0;
        $additionalDistance = max($effectiveDistance - $baseRadius, 0);
        $pricePerKm = $this->transfer_price_per_km ?? 0.0;
        $total = $basePrice + ($additionalDistance * $pricePerKm);

        return [
            'price' => $total,
            'formatted_price' => format_money($total),
            'route_distance_km' => round($effectiveDistance, 2),
            'pickup_distance_km' => $pickupDistance !== null ? round($pickupDistance, 2) : null,
            'dropoff_distance_km' => $dropoffDistance !== null ? round($dropoffDistance, 2) : null,
        ];
    }
    ...
    public function search($request)
    {
        ...
        if (is_numeric($pickupLat) && is_numeric($pickupLng)) {
            ...
            $query->whereNotNull($query->qualifyColumn('transfer_service_lat'));
            $query->whereNotNull($query->qualifyColumn('transfer_service_lng'));

            $latColumn = $query->qualifyColumn('transfer_service_lat');
            $lngColumn = $query->qualifyColumn('transfer_service_lng');
            $radiusColumn = $query->qualifyColumn('transfer_service_radius_km');

            $query->selectRaw(
                "(6371 * acos( cos(radians(?)) * cos(radians({$latColumn})) * cos(radians({$lngColumn}) - radians(?)) + sin(radians(?)) * sin(radians({$latColumn})) )) ) AS pickup_distance_km",
                [$pickupLat, $pickupLng, $pickupLat]
            );

            $query->havingRaw("({$radiusColumn} IS NULL OR {$radiusColumn} = 0 OR pickup_distance_km <= {$radiusColumn})");
        }
